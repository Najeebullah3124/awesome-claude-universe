...
describe("redactGitHubTokens", () => {
  it("should redact personal access tokens (ghp_)", () => {
    const token = "GHP_EXAMPLE_REDACTED";
  ...
  it("should redact OAuth tokens (gho_)", () => {
    const token = "GHO_EXAMPLE_REDACTED";
  ...
  it("should redact installation tokens (ghs_)", () => {
    const token = "GHS_EXAMPLE_REDACTED";
  ...
  it("should redact refresh tokens (ghr_)", () => {
    const token = "GHR_EXAMPLE_REDACTED";
  ...
  it("should handle tokens in code blocks", () => {
    const content = `\`\`\`bash
export GITHUB_TOKEN=GHP_EXAMPLE_REDACTED
\`\`\``;
    const expected = `\`\`\`bash
...
\`\`\``;
  ...
  it("should handle multiple tokens in one text", () => {
    const content =
      "Token 1: GHP_EXAMPLE_REDACTED and token 2: GHO_EXAMPLE_REDACTED";
  ...
  it("should handle tokens in URLs", () => {
    const content =
      "https://api.github.com/user?access_token=GHP_EXAMPLE_REDACTED";
    expect(redactGitHubTokens(content)).toBe(
  ...
  it("should not redact partial matches or invalid tokens", () => {
    const content =
      "This is not a token: ghp_short or GHO_EXAMPLE_REDACTED";
  ...
  it("should handle edge cases", () => {
    ...
    expect(redactGitHubTokens("GITHUB_PAT_EXAMPLE_REDACTED")).toBe("GITHUB_PAT_EXAMPLE_REDACTED");
  });
});
...
describe("sanitizeContent with token redaction", () => {
  it("should redact tokens as part of full sanitization", () => {
    const content = `
      <!-- Hidden comment with token: GHP_EXAMPLE_REDACTED -->
      Here's some text with a token: GHO_EXAMPLE_REDACTED
      And invisible chars: test\u200Btoken
    `;

    const sanitized = sanitizeContent(content);

    expect(sanitized).not.toContain("GHP_EXAMPLE_REDACTED");
    expect(sanitized).not.toContain("GHO_EXAMPLE_REDACTED");
